...
class Case2ExampleBot(UTCBot):
    ...
    async def handle_round_started(self):
        ...
        for strike in option_strikes:
            for option_type in ["C", "P"]:
                self.deltas[f"UC{strike}{option_type}"] = 0
    ...
    def compute_vol_estimate(self) -> float:
        ...
        return 0.35 # (WAS FOR THE EXAMPLE BOT, so I commented it out here)
    ...
    async def update_options_quotes(self):
        ...
        time_to_expiry = (21 + 5 - self.current_day) / 252 # updated logic
        ...
        for strike in option_strikes:
            for option_type in ["C", "P"]:
                ...
                theo = self.compute_options_price(
                    option_type, self.underlying_price, strike, time_to_expiry, vol
                )
                ########## ADDED ##########
                # calculate price threshold used in bid and ask orders
                c_bid_p_ask_thresh = round((thresh_val)*(self.pos_delta)+.30,1)
                c_ask_p_bid_thresh = round(-(thresh_val)*(self.pos_delta)+.30,1)
    ...
    async def handle_exchange_update(self, update: pb.FeedMessage):
        ...
        if kind == "pnl_msg":
            ...
            self.underlying_price = (
                float(book.bids[-1].px) + float(book.asks[-1].px)
            ) / 2
